...
public class CreateAntibiogramHandler(
    ...
    : ICommandHandler<CreateAntibiogramCommand, Result<CreateAntibiogramResult>>
{
    public async Task<Result<CreateAntibiogramResult>> HandleAsync(CreateAntibiogramCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (pathogen is null)
        {
            ...
            return Result<CreateAntibiogramResult>.Failure(new Error(Status.BadRequest, "Pathogen not found"));
        }
        ...
        if (!allAntibioticsExist)
        {
            ...
            return Result<CreateAntibiogramResult>.Failure(new Error(Status.BadRequest, "Not all antibiotic IDs exist"));
        }
        ...
        return Result<CreateAntibiogramResult>.Success(Status.Created, new CreateAntibiogramResult(antibiogram.Id));
    }
}
