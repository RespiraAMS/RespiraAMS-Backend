...
/// <summary>
/// Manager lifecycle endpoints (Create/Update/Delete). Admin only. Each endpoint starts the
/// corresponding Wolverine saga (the same Auth → Doctor → Media workflow used for doctors) but
/// provisions an account with <see cref="RoleType.Manager"/>. The request is accepted
/// (HTTP 202) and tracked asynchronously via <see cref="ProcessTrackerService"/>.
/// </summary>
[ApiController]
...
) : ControllerBase
{
    ...
    /// multipart/form-data: the avatar as <c>file</c> and the metadata as a JSON string in the
    /// <c>request</c> field. The avatar is uploaded to the Media service first; the returned
    /// media id is then used to start the saga. The created account always has role Manager.
    /// </summary>
    [HttpPost]
    [Route("create")]
    ...
    )
    {
        ...
        await trackerService.CreateAsync(sagaId, "CreateManager", dto.ManagerDoctorId, null, cancellationToken);
